...
;------------------------------------------------------------------
			...
			;--------------------------------------------------------------------------
			; Drawing Notes: 
			; - The game uses XOR drawing for a more flicker-free sprite ERASURE by 
...
GO_25FPS:	XOR A						; $6574  ; reset for next 25FPS test
			...
			CALL ANIMATE_TILES			; $659F		; animated icons (pump)
			...
			CALL GUN_EMPLACEMENTS		; $65B7		; Enemy static guns (plant,ball turret)
...
LOOP_KD:	CALL ANY_KEY_DOWN		; $65FF  ; all keys released
			JR NZ,LOOP_KD			; $6602
	
LOOP_GK:	CALL GET_KEY			; $6604  ; pause - wait for key press
			OR A					; $6607
			JR Z,LOOP_GK			; $6608
			RET						; $660A

;----------------------------------------------------
			; !!! Manual says: "To abort press 5, 6, 7, 8, 9 and 1 simultaneously." (this is incorrect) !!!
			; You infact need to press 1,2,3,4,5 to abort game.
INGAME_RESET:
				LD A, $F7          ; %11110111 (select key row 1-5)
				IN A, ($FE)        ; note: bits set to zero = pressed
				AND $1F            ; 
				RET NZ             ; exit, not all keys pressed
				JP MAIN            ; 0-4 are zero (all pressed)
;----------------------------------------------------
...
; -------------------------------------------------------------
; Get Any Key Press 
;   Returns key status in A: 0=at least one key is pressed,1=no keys are pressed
ANY_KEY_DOWN:   
		  XOR A    		 ; $6672  ; 0, Not a valid port, but works!
          IN A,($FE)     ; $6673  ; Read keyboard
          CPL            ; $6675  ; Invert bits (active low input)
          AND $1F        ; $6676  ; Mask to check any of the 5 lowest bits
          RET            ; $6678  ;

; NOTE: $00FE (undocumented speccy port)
; Unlike $7FFE (which detects keys 1,2,3,4 & 5 when A=$7F), this port appears to show if *any* key is pressed.
; 
; The hardware seems to detect if *any* key is down, 
; but does not distinguish between the 8 half-rows each 5 keys per row.
; -------------------------------------------------------------

; -------------------------------------------------------------
; Generate a random'ish value
; 	Input:  A=Maximum limit
;  	Output: A=Random number in range 0 to (A-1)
; 	Formula: RAND = (RAND + 7) * 13 (mod 65536)
GET_RAND_VALUE:		
			PUSH HL						; $6679
			PUSH DE						; $667A
			PUSH BC						; $667B
			INC A						; $667C
			PUSH AF						; $667D
			LD HL,(RAND_STATE)			; $667E  ; Get seed
			LD DE,$0007					; $6681
			ADD HL,DE					; $6684  ; (RAND + 7)
			LD E,L						; $6685
			LD D,H						; $6686
			ADD HL,HL					; $6687  ; x2 
			ADD HL,HL					; $6688	 ; x4
			...
			ADD HL,BC					; $668C  ; x12
			ADD HL,DE					; $668D  ; x13
			LD (RAND_STATE),HL			; $668E  ; Keep seed
			LD A,L						; $6691
			XOR H						; $6692  ; Mix high/low bytes 
			LD E,A						; $6693
			LD D,$00					; $6694  ; Need to use DE as 8-bit value later
			POP AF						; $6696
			LD HL,$0000					; $6697  ; Zero - About to hold ongoing sum
			LD B,$08					; $669A  ; Counter for 8-bit division
RND_LOOP:	
			ADD HL,HL					; $669C  ; x2
			RLA							; $669D  ; Get carry
			JR NC,SKIP_CARRY			; $669E
			ADD HL,DE					; $66A0
SKIP_CARRY:		
			DJNZ RND_LOOP				; $66A1
			LD A,H						; $66A3  ; Final scaled number
			POP BC						; $66A4
			POP DE						; $66A5
			POP HL						; $66A6
			RET							; $66A7

; Random state storage (2-byte variable)
RAND_STATE:			defw 	$0000 	; $66A8 

;---------------------------------------------------------------------
; Clear Screen and Attribute Memory (works backwards)
CLR_SCREEN:	
			LD C,$00				; $66AA
			LD HL,$5AFF				; $66AC
			LD DE,$5AFE				; $66AF
...
FILLOUT_INGAME_TABLE:
			...
			CALL NEXT_SCREEN_LINE		; $6714	; Screen Address Calculation
			INC IX					; $6717
			INC IY						; $6719
...
LOOKUP_6300_ADDR_OFFSET:		
			...
			RET						; $674B
;---------------------------------------------------------------------
; Get the HL register pair to the next screen line
NEXT_SCREEN_LINE:
			INC H				; $674C  ; Next row 
			LD A,H				; $674D
			AND $07				; $674E  ; Check if crossed third boundary
			RET NZ				; $6750  ; Exit if still in same third
			LD A,L				; $6751
			ADD A,$20			; $6752  ; Move to next screen third
			LD L,A				; $6754
			RET C				; $6755  ; Exit if carried to new third
			LD A,H				; $6756
			SUB $08				; $6757  ; Third transition
			LD H,A				; $6759
			RET					; $675A
;---------------------------------------------------------------------
; Get the HL register pair to the previous screen line.
PREVIOUS_SCREEN_LINE:
			LD A,H				; $675B
			DEC H				; $675C  ; Previous row
			AND $07				; $675D  ; Check if crossing third boundary
			RET NZ				; $675F  ; Exit if still in same third
			LD A,L				; $6760
			SUB $20				; $6761  ; Move to previous screen third
			LD L,A				; $6763
			RET C				; $6764  ; Exit if carried to new third
			LD A,H				; $6765 
			ADD A,$08			; $6766  ; Third transition
			LD H,A				; $6768
			RET					; $6769
;---------------------------------------------------------------------
; Gives 21cycles x BC  (ignoring setup cost)
WAIT_21xBC:  
			PUSH BC				; $676A
			PUSH DE				; $676B
			PUSH HL				; $676C
...
GET_TILE_ADDR_OFFSET:
			...
			RET					; $678E
;---------------------------------------------------------------------
GET_CACHED_TILE_AS_HL:
			PUSH DE						; $678F
			CALL GET_TILE_ADDR_OFFSET	; $6790   
			LD DE,$5F00					; $6793	 ; Base address 
			ADD HL,DE					; $6796  ; Final address 
			POP DE						; $6797
			RET							; $6798
;---------------------------------------------------------------------
GET_COLOUR_ATT_AS_HL:
			PUSH DE						; $6799
			CALL GET_TILE_ADDR_OFFSET	; $679A  ; HL=offset based on Y/X (seeing DE=$2000 before call)
...
RESTORE_COLOUR:
			PUSH DE						; $67A3
			CALL GET_TILE_ADDR_OFFSET	; $67A4
			LD DE,$5B00					; $67A7
			ADD HL,DE					; $67AA
			POP DE						; $67AB
			RET							; $67AC
;---------------------------------------------------------------------
COPY_COLOUR_ATTRIBUTES:
			LD HL,$5800				; $67AD
			LD DE,$5B00				; $67B0 ; RAM just after screen
			LD BC,$0300				; $67B3
			LDIR					; $67B6
			RET						; $67B8
;---------------------------------------------------------------------
SET_COLLISION_BOX:		
			...
			LD L,A						; $67C0
			LD H,$00					; $67C1
			LD BC,COLLISION_BOXES_TABLE	; $67C3  ; base
			ADD HL,BC					; $67C6  ; 
			PUSH HL						; $67C7
			POP IX						; $67C8  ; IX=HL
			LD A,(IX+$02)				; $67CA
			LD (LABEL_X2_CHECK+1),A		; $67CD
			ADD A,(IX+$00)				; $67D0
			NEG							; $67D3
			INC A						; $67D5		
			LD (LABEL_X1_CHECK+1),A		; $67D6
			LD A,(IX+$03)				; $67D9
			LD (LABEL_Y2_CHECK+1),A		; $67DC
			ADD A,(IX+$01)				; $67DF
			NEG							; $67E2
			INC A						; $67E4
			LD (LABEL_Y1_CHECK+1),A		; $67E5 
			POP IX						; $67E8
			POP HL						; $67EA
			POP BC						; $67EB
			POP AF						; $67EC
			RET							; $67ED
; -------------------------------------------------------------
; These are the five preset hit boxes (code calcs/negates first two values)
; For Example the code adjusts the boxes as (noting X is half-scale):- 
; (-7x,  -15y, 4x, 8y) 
; (-15x, -31y, 8x, 16y)
; (-11x, -23y, 4x, 8y)
; (-8x,  -16y, 1x, 1y)
; (-7x,  -15y, 8x, 16y)
COLLISION_BOXES_TABLE:			
			defb $04,$08,$04,$08		; 67EE ; box 00 - ???
			defb $08,$10,$08,$10		; 67F2 ; box 01 - hitbox for pick ups
			defb $08,$10,$04,$08   		; 67F6 ; box 02 - hitbox for seeker
			defb $08,$10,$01,$01		; 67FA ; box 03 - hitbox for small bullets 
			defb $00,$00,$08,$10	   	; 67FE ; box 04 - hitbox for bombs (rockets)

; -------------------------------------------------------------
; Checks if (C=X, B=Y) is inside an 8×16 hitbox from (E=X, D=Y)
; Note: Games X valus are at 1/2 scale
CHECK_POINT_IN_HITBOX:
			LD A,E				; $6802  ;  hitbox X position
			SUB C				; $6803
LABEL_X2_CHECK:
			SUB $08				; $6804  ; hitbox width
LABEL_X1_CHECK:			
			CP $F0				; $6806  ; comparing difference 
			LD A,$00			; $6808 
			RET C				; $680A  ; No hit
			LD A,D				; $680B
			SUB B				; $680C
LABEL_Y2_CHECK:			
			SUB $10				; $680D  ; Hitbox height
LABEL_Y1_CHECK:	 
			CP $E0				; $680F  ; comparing difference 
			LD A,$00			; $6811  ; No hit
			RET C				; $6813
			INC A				; $6814  ; set collision 
			RET					; $6815  ; A with collision
; -------------------------------------------------------------
...
NO_GRAV:	LD A,(PLR_MOVEMENT_FLAG)	; $6944
			...
			LD BC,(POS_XY)				; $694B
			LD (POS_XY),DE				; $694F
			; Get sprite frame (facing PLR_DIRECTION)
L_6953:		LD A,(PLR_DIRECTION)			; $6953  ;
			LD L,$01					; $6956
			INC A						; $6958
...
PLR_DIRECTION:			defb $01 				; $696B ;  ships facing PLR_DIRECTION
PLR_MOVE_YPOS			defb $02      			; $696C ;
...
CHECK_SCENE_RIGHT:		
			...
			CALL GET_CACHED_TILE_AS_HL	; $6983
...
CHECK_SCENE_LEFT:		
			...
			CALL GET_CACHED_TILE_AS_HL				; $69AC
			DEC L				; $69AF
			LD BC,$0020				; $69B0
			LD A,(HL)				; $69B3
			ADD HL,BC				; $69B4
...
CHECK_SCENE_DOWN:		
			...
			CALL GET_CACHED_TILE_AS_HL				; $69CE
			LD BC,$0040				; $69D1
			ADD HL,BC				; $69D4
			LD A,(HL)				; $69D5
			INC L				; $69D6
...
CHECK_SCENE_UP:		
			...
			CALL GET_CACHED_TILE_AS_HL			; $69F0
			LD BC,$FFE0			; $69F3
			ADD HL,BC			; $69F6
			LD A,(HL)			; $69F7
			INC L				; $69F8
...
LOOP_SCENE_COLLISION:
			...
			LD ($67FF),A				; $6CC2
			LD A,$04				; $6CC5
			CALL SET_COLLISION_BOX				; $6CC7
			LD E,(IX+$00)				; $6CCA
			LD D,(IX+$01)				; $6CCD
			CALL CHECK_POINT_IN_HITBOX				; $6CD0
			OR A				; $6CD3
...
L_6D7A:		PUSH DE				; $6D7A
			PUSH BC				; $6D7B
			CALL GET_CACHED_TILE_AS_HL				; $6D7C
L_6D7F:		LD A,(HL)				; $6D7F
			OR A				; $6D80
			CALL NZ,L_6D96				; $6D81
			INC HL				; $6D84
...
L_6E95:
			...
			JR L_6E95					; $6E9E
L_6EA0:
			; random volcano rocks
			LD A,$01					; $6EA0
			CALL GET_RAND_VALUE			; $6EA2
			ADD A,$02					; $6EA5  ; 2 to 3
			LD (HL),A					; $6EA7  ; store sprite type
			...
			LD (HL),D					; $6EAC  ; ypos
			INC HL						; $6EAD
			CALL CREATE_EJECT_VELOCITY	; $6EAE  ; returns BC = mY/mY
			LD (HL),C					; $6EB1  ; mx
			INC HL						; $6EB2
			LD (HL),B					; $6EB3  ; my
			EX AF,AF'					; $6EB4
...
DEBRIS_SPRITES:
			LD HL,DEBRIS_DATA		; $7069   ; Explosive data
LOOP_DEBRIS:
			LD A,(HL)				; $706C
			LD B,A					; $706D
			CP $FF					; $706E   
...
SLOT_READY:
			...
			LD D,(HL)				; $707F
			LD A,B					; $7080
			CALL DRAW_SPRITE16X8	; $7081  ; clear old sprite
			INC HL					; $7084
			LD A,E					; $7085
			ADD A,(HL)				; $7086
			LD E,A					; $7087
			CP $78					; $7088
			JR NC,OFF_SCREEN 		; $708A
			INC HL					; $708C
			INC (HL)				; $708D
			LD A,(HL)				; $708E
			ADD A,D					; $708F
...
OFF_SCREEN:
			POP HL					; $70AA
			POP HL					; $70AB
			LD (HL),$00				; $70AC  ; slot free
			LD BC,$0006				; $70AE  ; next item
			ADD HL,BC				; $70B1
			JR LOOP_DEBRIS			; $70B2
...
MAP_LEVEL_LOOKUP:  
				defw MAP01,MAP02,MAP03,MAP04,MAP05,MAP06,MAP07,MAP08,MAP09,MAP10  
				defw MAP11,MAP12,MAP13,MAP14,MAP15,MAP16,MAP17,MAP18,MAP19,MAP20
				defw MAP21,MAP22,MAP23,MAP24,MAP25,MAP26,MAP27,MAP28,MAP29,MAP30
				defw MAP31,MAP32,MAP33,MAP34,MAP35,MAP36,MAP37,MAP38,MAP39,MAP40
				defw MAP41,MAP42,MAP43,MAP44,MAP45,MAP46,MAP47,MAP48,MAP49,MAP50
				defw MAP51,MAP52,MAP53,MAP54,MAP55,MAP56,MAP57,MAP58,MAP59,MAP60
				defw MAP61,MAP62,MAP63,MAP64,MAP65,MAP66,MAP67,MAP68,MAP69,MAP70
				defw MAP71,MAP72,MAP73,MAP74,MAP75						
				;-------------------------------------------------------------------
...
RESET_FOR_NEXT_SCREEN:		
	...
	;	LD (LEVEL_OFFSET),a
		;	ld a,$2B
		;	ld a,3
		;	LD A,$4A	; end lift level
			LD A,(LEVEL_INDEX)				; $7263 ; Index (each entry is 2 bytes)
			LD BC,MAP_LEVEL_LOOKUP			; $7266 ; get level lookup table
			CALL GET_ADR_FROM_TABLE			; $7269 ; 0:MAP01,1:MAP02 74:MAP75
			;----------------------------------------
...
UPDATE_INGAME_TILES:	
			...
			; Store Tile for collision lookup
			CALL GET_CACHED_TILE_AS_HL		; $727F
			LD (HL),A						; $7282  ; Copy Tile Map (base at $5F00)
			POP HL							; $7283
...
SPECIAL_TITLES:		
			...
			; Store Tile for collision lookup
			CALL GET_CACHED_TILE_AS_HL		; $72A9
...
L_72EC:	
			...
			ADD A,$10						; $7306  ; 16
			LD D,A							; $7308
			LD E,$00						; $7309  ; reset Xpos
			JP TILE_SETUP_LOOP				; $730B
			; --------------------------------------------------
L_730E:		LD (IX+$00),$FF					; $730E
			CALL COPY_COLOUR_ATTRIBUTES		; $7312
			CALL L_9BD8						; $7315
			LD A,$01						; $7318
			LD ($9BD3),A					; $731A
			LD DE,(POS_XY)					; $731D
...
FOUND_ENTRY:			
			...
			LD BC,LIST_OF_TABS_FOR_SETUP	; $73C9
			ADD HL,BC						; $73CC  ; add offset
			PUSH HL							; $73CD
			POP IY							; $73CE  ; IY has struct pointer
			; get table from list
			LD L,(IY+$02)					; $73D0  ; table
			LD H,(IY+$03)					; $73D3
			; write x/y coords to table
			LD (HL),E						; $73D6  ; Starting Xpos
			INC HL							; $73D7
			LD (HL),D						; $73D8  ; Starting Ypos
			INC HL							; $73D9
			...
			; Store write location of current table
			LD (IY+$02),L					; $73E4  
			LD (IY+$03),H					; $73E7
			POP BC							; $73EA
			POP HL							; $73EB

			INC HL							; $73EC  ; next group key 
...
MAP_TILE_TO_GROUP:
			...
			; This list indes into 5 slots of LIST_OF_TABS_FOR_SETUP
			defb $2E,$00,$2F,$00,$8D,$00,$8E,$00,$8F,$00,$90,$00    ; index0: Indexs for Destroyable Titles
			defb $27,$01,$32,$01,$52,$01,$94,$01,$98,$01			; index1: Indexs for Emplacements
			defb $46,$02,$47,$02									; index2: Enemy Rockets
			defb $81,$03											; index3: Volcano
			defb $83,$04											; index4: Level Complete Landing Platform
			defb $FF	
			;--------------------------------------------------------------------

LIST_OF_TABS_FOR_SETUP:
			defw DESTROYABLE_TILE_DATA		; $7413  ; index0
			defb $00,$00,$02				; $7415  ;
			defw EMPLACEMENT_LIST 					 ; index1
			defb $00,$00,$02				; $741A  ;
			defw PROXIMITY_ROCKET_DATA				 ; index2
			defb $00,$00,$02						 ;
			defw VOLCANO_LIST						 ; index3
			defb $00,$00,$02				; $7424	 ;
			defw LEVEL_COMPLETE_DATA				 ; index4
			defb $00,$00,$02                      	 ; 2bytes table ,stride		
			defb $00,$00					; zero pair to terminate

			; ------------------------------------------------
RESET_TABLE_LIST:		
			LD BC,$0005						; $742E  ; struct size
			LD IX,LIST_OF_TABS_FOR_SETUP	; $7431
RESET_TABLE_LOOP:
			; Get table location
			LD A,(IX+$00)				; $7435
			LD L,A						; $7438
...
SETUP_LEVEL:
			...
			;------------------------------------------------
			; Set Players Starting PLR_DIRECTION 
			INC HL								; $7493
			LD A,(HL)							; $7494f
			LD (PLR_DIRECTION),A					; $7495
			;------------------------------------------------
			XOR A								; $7498
			LD (EGG_TIMER),A					; $7499
...
; for example: (256/2)-8,(192/2)-8 will put the ship centre Y but far right screen edge
			;------------
START_POS:	defb $20,$90  ; X=32 (64pixels), Y=144
			defb $06,$00  ; Next Offset, TableIndex
			defb $F4,$01  ; Timer=500 
			defb $01,$00  ; PLR_DIRECTION=Right, Unused
			;------------
			defb $60,$50  ; X, Y
			defb $06,$15  ; Next Offset, TableIndex
			defb $EE,$02  ; Timer
			defb $FF,$00  ; PLR_DIRECTION=Left, Unused
			;------------
			defb $60,$60  ; X, Y
			defb $08,$2B  ; Next Offset, TableIndex
			defb $84,$03  ; Timer
			defb $FF,$00  ; PLR_DIRECTION=Left, Unused       
			;------------
...
L_7502:		LD E,A						; $7502	; Dx
			...
			OR A						; $750A
			JR Z,L_752F					; $750B
			CALL GET_CACHED_TILE_AS_HL	; $750D
			LD A,(HL)					; $7510
			OR A						; $7511
			JR Z,L_752F					; $7512
...
ADD_VOLCANO_LOOP:		
			LD A,(HL)					; $7587 ; get Xpos, $FF terminate 
			CP $FF						; $7588
			RET Z						; $758A
			LD E,A						; $758B 
			INC E						; $758C
			...
			LD C,A						; $759C  ; colour
			; ------------
			CALL ADD_DEBRIS				; $759D
			JP ADD_VOLCANO_LOOP			; $75A0

VOLCANO_LIST:  ; x10 items [x,y]
			defb $49,$54,$45,$20,$41,$44,$44,$52	; $75A3
			defb $45,$53,$53,$0D,$0A,$09,$45,$58	; $75AB
			defb $58,$0D,$0A,$0D,$FF                ; $75B3  ; $FF terminate
...
BAD_LUCK_TXT:												; $7730
			...
			; ---------------------------------------------
			; Map Level 75 - END BONUS LEVEL
MAP75:		defb $FF,$06							; $7791
			...
			defb $00,$87,$00,$46,$46,$FF,$05,$00	; $77BB 
			defb $06,$07,$08,$00,$88,$89,$8A,$88	; $77C3 
			defb $00,$4C,$4D,$46,$00,$00,$FF,$05	; $77CB 
			defb $14,$9A,$FF,$04,$9B,$9C,$FF,$05	; $77D3 
			defb $14,$FF,$30,$00,$E5,$CD,$44,$11	; $77DB 
			defb $C2,$B5,$0A,$CD,$EF,$0B,$CD,$E1	; $77E3 
			defb $0B,$E1,$18,$E8,$19,$11,$00,$40	; $77EB 
			defb $19,$E5,$36,$FA,$23,$35,$23,$35	; $77F3 
			defb $97,$32,$6E,$0A                    ; $77FB 
			; ---------------------------------------------

DO_TOP_BAR:
			LD HL,TOP_BAR_DRAW_LIST	; $77FF
			CALL DRAW_LIST			; $7802  ; draw top bar

			CALL UPDATE_SCORE_TXT	; $7805  ; Score
			CALL UPDATE_GEMS_TXT	; $7808  ; Gems Collected
			CALL UPDATE_LIVES_TXT	; $780B  ; Lives
			CALL UPDATE_EGG_TIMER	; $780E  ; Egg Timer
			JP UPDATE_WEAPONS_TXT	; $7811  ; (Uses a final Ret)
...
TOP_BAR_DRAW_LIST:
			...
			defb $D5,$07,$78,$02,$DA,$08,$D2,$09		; $784B
			defb $DF,$01,$18,$DA,$0E,$D2,$0F,$AD		; $7853
			defb $10,$D9,$11,$DF,$01,$00,$DC,$1E		; $785B
			defb $AE,$1E,$DF,$01,$1F,$DC,$1E,$AE		; $7863
			defb $1E,$DF,$00,$05,$E3,$7F,$78,$DF		; $786B
			defb $00,$0E,$E3,$7F,$78,$DF,$00,$17		; $7873
			defb $E3,$7F,$78,$FF,$DE,$1F,$D4,$23		; $787B
...
COUNT_DIGIT_DONE:
			...
			CALL ICON8x8			; $79A3	 ; Display 8X8 icon
			INC E				; $79A6	 ; X coords 
L_79A7:
			POP AF				; $79A7
			RET					; $79A8
...
UPDATE_EGG_TIMER:		LD HL,$79BE			; $79A9  ; data address
			CALL DRAW_LIST			; $79AC
...
DO_EGG_TIMER:		
			...
			JP UPDATE_EGG_TIMER							; $7A18  ;Update display
...
L_7A88:		ADD A,E						; $7A88
			LD E,A						; $7A89
			LD A,(PLR_DIRECTION)			; $7A8A ; $FF=(-)left, $01=(+)right (bullet velocity)
...
FIND_UNUSED_LOOP:		
			...
			POP DE						; $7ADA
			CALL GET_CACHED_TILE_AS_HL	; $7ADB
			LD A,(HL)					; $7ADE
			OR (HL)						; $7ADF
			POP HL						; $7AE0
...
CHECK_BULLETS_LOOP:		
			...
			EX AF,AF'					; $7B04
			PUSH HL						; $7B05
			CALL GET_CACHED_TILE_AS_HL	; $7B06
...
TABLE_JMP_BOMBS: 	; rockets
			LD HL,BOMBS_ARC_DOWN					; $7C5C  ; Fire rockets up
			LD A,(PLR_MOVE_YPOS)				; $7C5F
			CP $02								; $7C62	
			JP Z,BOMB_IS_DOWN							; $7C64
			LD HL,BOMBS_ARC_UP					; $7C67	 ; Fire rockets down
BOMB_IS_DOWN:		
			LD IX,BOMB_DATA						; $7C6A  
			; BOMB_DATA 7-bytes:
			; 00: XPos, 01: YPos , 02: Countdown Timer , 03-04: pointer to arc table 
			; 05: X-Movement ($02=right, $FD=left) , 06: Facing Left or Right (for GFX)
			LD A,(GAME_COUNTER_8BIT)			; $7C6E
			AND $03								; $7C71  ; Only create rockets every 4th frame
			JP NZ,UPDATE_SUPER_WEAPON_DIGITS	; $7C73

			LD BC,$0007							; $7C76  ; Rocket slot size = 7 bytes
BOMB_DISPLAY_LOOP:	
			LD A,(IX+$00)						; $7C79
			CP $FF								; $7C7C  ; End marker
			JP Z,UPDATE_SUPER_WEAPON_DIGITS		; $7C7E
			LD A,(IX+$02)						; $7C81
			OR A								; $7C84  ; slot free
			JR Z,L_7C8C							; $7C85
			ADD IX,BC							; $7C87
			JP BOMB_DISPLAY_LOOP 				; $7C89
			; --------------------------------------------
L_7C8C:		LD A,$02							; $7C8C
			...
			POP DE							; $7C9D
			; --------------------------------------------
			LD (IX+$02),$10					; $7C9E  ; Set rocket timer (16 frames)
			LD (IX+$03),L					; $7CA2  ; arc table
			LD (IX+$04),H					; $7CA5
			LD HL,(POS_XY)					; $7CA8  ; Get player position
			LD DE,$0402						; $7CAB  ; Position offset (x+4, y+2)
			ADD HL,DE						; $7CAE  ; rocket start position
			LD (IX+$00),L					; $7CAF  ; Store X
			LD (IX+$01),H					; $7CB2  ; Store Y
			LD A,(PLR_DIRECTION)			; $7CB5  ; facing direction
			ADD A,A							; $7CB8  ; double x-movement
			LD (IX+$05),A					; $7CB9
			EX DE,HL						; $7CBC
			CP $FE							; $7CBD  ; Left ???
			LD A,$01						; $7CBF
...
UPDATE_PLR_BOMBS:		
			LD A,$02						; $7CD2
			CALL SET_COLLISION_BOX	; $7CD4
			LD IX,BOMB_DATA					; $7CD7
L_7CDB:		LD A,(IX+$00)					; $7CDB
			CP $FF							; $7CDE  ; End marker
			RET Z							; $7CE0
...
BOMB_LOOP:			
			...
			ADD IX,BC						; $7CEA
			JP L_7CDB						; $7CEC
L_7CEF:		LD C,(IX+$02)					; $7CEF  ; Countdown
			LD B,$00						; $7CF2
			LD L,(IX+$03)					; $7CF4
			LD H,(IX+$04)					; $7CF7
			DEC HL							; $7CFA  ; Move arc data
			ADD HL,BC						; $7CFB  ; Countdown as offset
			DEC C							; $7CFC

			; Stop countdown at zero, reusing the final largest arc value
			JR Z,SKIP_ZERO					; $7CFD  ; skip storing when zero
			LD (IX+$02),C					; $7CFF
SKIP_ZERO:			
			LD E,(IX+$00)					; $7D02
			LD D,(IX+$01)					; $7D05
			...
			JR NC,L_7D15					; $7D0B
			; ----------------------------------------
			; Sparkle effect every other frame
			LD A,(GAME_COUNTER_8BIT)		; $7D0D
			AND $01							; $7D10
			CALL Z,SPARKLE_EFFECT			; $7D12
			; -----------------------------------------
L_7D15:		LD A,(IX+$06)					; $7D15  ; Facing direction
			CALL DRAW_SPRITE16X8			; $7D18  ; clears last rocket drawn
			LD A,(IX+$05)					; $7D1B  ; get x movement
 			LD B,A							; $7D1E  ; 

			PUSH HL							; $7D1F
			CALL GET_CACHED_TILE_AS_HL		; $7D20
			LD A,(HL)						; $7D23
			POP HL							; $7D24
			OR A							; $7D25
			JR NZ,BOMB_COLLISION			; $7D26  ; Hit tile
			LD A,B							; $7D28
			ADD A,E							; $7D29  ; Xpos
			CP $7C							; $7D2A  
			JR NC,BOMB_COLLISION			; $7D2C  ; off screen right
			LD E,A							; $7D2E
			LD (IX+$00),A					; $7D2F  ; Update Xpos
			LD A,(IX+$01)					; $7D32  ; Get Ypos
			LD D,A							; $7D35
			CALL SCENE_COLLISION_BOTTOM		; $7D36
			JR NZ,BOMB_COLLISION			; $7D39
			LD A,(HL)						; $7D3B  ; Y delta
			ADD A,D							; $7D3C  ; Update Ypos
			LD D,A							; $7D3D
			CP $B8							; $7D3E
			JR NC,BOMB_COLLISION			; $7D40  ; off screen bottom
			CALL ENEMY_COLLISIONS			; $7D42
			JR NZ,BOMB_COLLISION			; $7D45
			; ----------------------------------------------------
			; Draw Bomb (rocket)
			LD (IX+$01),D					; $7D47  ; Update Ypos
			LD A,(IX+$06)					; $7D4A  ; facing
			LD C,$47						; $7D4D  ; colour
			CALL COLOUR_SPRITE				; $7D4F
			CALL DRAW_SPRITE16X8			; $7D52
			; ----------------------------------------------------
   			JP BOMB_LOOP					; $7D55
				; -------------------------------------------------
BOMB_COLLISION:
			; free bomb for reuse after collision
			XOR A							; $7D58
			LD (IX+$02),A					; $7D59  ; Mark slot inactive
...
L_7DDF:		LD A,(HL)								; $7DDF
			...
			OR A									; $7DEA
			JR Z,L_7DDF								; $7DEB
			CALL CHECK_POINT_IN_HITBOX				; $7DED
			OR A									; $7DF0
...
L_7F25:		LD A,(HL)				; $7F25
			...
			CP $FE					; $7F37
			JR Z,L_7F45				; $7F39
			CALL SCENE_COLLISION_RIGHT				; $7F3B
			OR A					; $7F3E
			CALL NZ,L_7F97			; $7F3F
			JP L_7F4C				; $7F42
L_7F45:		CALL SCENE_COLLISION_LEFT				; $7F45
			OR A					; $7F48
			CALL NZ,L_7F97			; $7F49
			; --------------------------------------
...
L_7F4C:		LD A,B					; $7F4C
			CP $FC					; $7F4D
			JR Z,L_7F5B				; $7F4F
			CALL SCENE_COLLISION_BOTTOM				; $7F51
			OR A					; $7F54
			CALL NZ,L_7FBF			; $7F55
			JP L_7F62				; $7F58
L_7F5B:		CALL SCENE_COLLISION_TOP				; $7F5B
			OR A					; $7F5E
			CALL NZ,L_7FBF			; $7F5F
			; --------------------------------------
...
DO_SEEKER:		
			...
			LD B,(IX+$01)						; $8072
			LD H,$05							; $8075
L_8077:		CALL BULLET_MOVEMENT							; $8077
			JR Z,L_809E							; $807A
			DEC H								; $807C
			JR NZ,L_8077						; $807D
			LD L,(IX+$0A)						; $807F
...
SCENE_COLLISION_RIGHT:
			...
			LD A,$00					; $811F
			JR NZ,NO_HIT_R				; $8121
			CALL GET_CACHED_TILE_AS_HL	; $8123
			INC L						; $8126
			LD BC,$0020					; $8127
			LD A,(HL)					; $812A
			LD E,A						; $812B
...
SCENE_COLLISION_LEFT:
			...
			AND $03						; $8141
			LD A,$00					; $8143
			JR NZ,NOT_HIT_L				; $8145
			CALL GET_CACHED_TILE_AS_HL	; $8147
			DEC L						; $814A
			LD BC,$0020					; $814B
			LD A,(HL)					; $814E
			LD E,A						; $814F
...
SCENE_COLLISION_BOTTOM:
			...
			LD A,$00					; $8167
			JR NZ,NO_HIT_B				; $8169
			CALL GET_CACHED_TILE_AS_HL	; $816B
			LD BC,$0020					; $816E
			ADD HL,BC					; $8171
			LD A,(HL)					; $8172
			LD D,A						; $8173
...
SCENE_COLLISION_TOP:
			...
			LD A,$00					; $818B
			JR NZ,NO_HIT_T				; $818D
			CALL GET_CACHED_TILE_AS_HL	; $818F
			LD BC,$FFE0					; $8192
			ADD HL,BC					; $8195
			LD A,(HL)					; $8196
			LD D,A						; $8197
...
; CALL DRAW_LIST		
			...
			OR A						; $81B5
			JR NZ,SKIP_1BIT_MUSIC		; $81B6
			LD A,(BEEPER_MUSIC_USED)	; $81B8  ; play beeper music just once when back at menu screens
			OR A						; $81BB
			JR NZ,SKIP_1BIT_MUSIC		; $81BC
			DI							; $81BE
			CALL START_BEEP_MUSIC		; $81BF
			CALL BEEP_MUSIC_LOOP		; $81C2
			EI							; $81C5
			LD A,$01					; $81C6
			LD (BEEPER_MUSIC_USED),A	; $81C8 ; 48k
 
SKIP_1BIT_MUSIC:		
			CALL ANY_KEY_DOWN			; $81CB
			JP NZ,SKIP_1BIT_MUSIC		; $81CE
...
INVALID_KEY_PRESS:			
			...
			LD A,B						; $822A
			OR C						; $822B
			JP NZ,MENU_LOOP				; $822C
			CALL DRAW_HI_SCORES					; $822F   ; Hi-Scores
			JP DO_MENU					; $8232
...
L_82E5:		PUSH BC							; $82E5
			...
			LD D,A							; $82EE
			LD E,$2C						; $82EF
			CALL GET_COLOUR_ATT_AS_HL		; $82F1
			LD A,($830B)					; $82F4
			INC A							; $82F7
			LD ($830B),A					; $82F8
			AND $07							; $82FB
...
L_8553:
			...
			DEC HL					; $855A
			DEC HL					; $855B
			CALL NEXT_SCREEN_LINE		; $855C
			EX DE,HL				; $855F
			CALL PREVIOUS_SCREEN_LINE				; $8560
...
KEY_SEARCH:		
			...
			; -------------------------------------------
FOUND_KEY:		
			LD IX,ANIM_TABLE				; $8762
SEARCH_UNUSED:
			BIT 7,(IX+Xpos)					; $8766  ; (includes status, bit7)
			JR NZ,L_87A5					; $876A
			EX AF,AF'						; $876C
			LD A,(IX+Ypos)					; $876D  ; already setup this anim
			OR A							; $8770
			JR Z,L_877B						; $8771
			EX AF,AF'						; $8773
			LD BC,$000A						; $8774  ; Struct Size (10)
			ADD IX,BC						; $8777
			JR SEARCH_UNUSED				; $8779
L_877B:		EX AF,AF'						; $877B
			PUSH HL							; $877C
			CALL GET_16X16TILE_GFX_AS_HL	; $877D  ; get gfx for tile
			LD (IX+TileGfxLow),L			; $8780	 ; store gfx
			LD (IX+TileGfxHigh),H			; $8783
			POP HL							; $8786
			LD (IX+Xpos),E					; $8787	 ; store 
			LD (IX+Ypos),D					; $878A
			INC HL							; $878D
			LD A,(HL)						; $878E
			LD (IX+StartingFrameLow),A		; $878F
			LD (IX+FrameLow),A				; $8792
			INC HL							; $8795
			LD A,(HL)						; $8796
			LD (IX+StartingFrameHigh),A		; $8797
			LD (IX+FrameHigh),A				; $879A
			INC HL							; $879D
			LD A,(HL)						; $879E
			LD (IX+InitCounter),A			; $879F
			LD (IX+Counter),A				; $87A2
L_87A5:		POP IX							; $87A5
			POP HL							; $87A7
			POP DE							; $87A8
			POP BC							; $87A9
...
ANIMATE_TILES:
			LD IX,ANIM_TABLE				; $87AC  ; 
TILE_ANIM_LOOP:		
			LD E,(IX+Xpos)					; $87B0  ; Xpos
			BIT 7,E							; $87B3  ; 
			RET NZ							; $87B5  ; 
			...
			LD D,A							; $87BB  ; Ypos 
			LD C,E							; $87BC  ; 
			CALL GET_CACHED_TILE_AS_HL		; $87BD  ; 
			LD E,C							; $87C0  ; 
			LD A,(HL)						; $87C1  ;
			OR A							; $87C2  ;
			JP Z,NEXT_ANIM_SPRITE			; $87C3  ; 

			LD D,(IX+Ypos)					; $87C6  ; 
			LD B,D							; $87C9  ; 
			LD A,(IX+Counter)				; $87CA  ;
			OR A							; $87CD  ; 
			JR NZ,HANDLE_COUNTDOWN 			; $87CE  ; 

			LD A,(IX+InitCounter)			; $87D0  ; 
			LD (IX+Counter),A				; $87D3  ;
			LD L,(IX+FrameLow)				; $87D6  ;
			LD H,(IX+FrameHigh)				; $87D9
			LD A,(HL)						; $87DC  ; 
			CP $FF							; $87DD  ; 
			JR NZ,UPDATE_FRAME				; $87DF  ; 
			
			; Reset to start of animation
			LD L,(IX+StartingFrameLow)		; $87E1  ; 
			LD H,(IX+StartingFrameHigh)	; $87E4
			LD (IX+FrameLow),L				; $87E7  ; 
			LD (IX+FrameHigh),H				; $87EA
			LD A,(HL)						; $87ED  ;
UPDATE_FRAME:		
			INC HL							; $87EE  ; 
			LD (IX+FrameLow),L				; $87EF  ; 
			LD (IX+FrameHigh),H				; $87F2  ; 

			; Update sprite graphics
			LD L,(IX+TileGfxLow)			; $87F5
			LD H,(IX+TileGfxHigh)			; $87F8
			CALL DRAW4X4SPRITE				; $87FB  ;
			LD (IX+TileGfxLow),L			; $87FE
			LD (IX+TileGfxHigh),H			; $8801
			LD B,A							; $8804 ; 
			CALL GET_CACHED_TILE_AS_HL		; $8805 ; 
			LD (HL),B						; $8808 ; 
HANDLE_COUNTDOWN:		
			DEC (IX+Counter)				; $8809 ; 
NEXT_ANIM_SPRITE:		
			LD DE,$000A						; $880C	; 
			ADD IX,DE						; $880F ; 
			JP TILE_ANIM_LOOP				; $8811 ;


ANIM_TABLE:
			defb $FC,$00,$00,$54,$FC,$A8,$00,$54,$FC,$00    ; $8814
			defb $00,$54,$A8,$00,$00			
			...
			defb $C3,$C3,$C3,$FF							; $8843

DATA_ANIM:
			defb $1D,$60,$88,$03			; $8847 ; $1D=frame, anim frames lookup=$8860, $03=frame pause rate
			defb $20,$64,$88,$03			;
			defb $1F,$68,$88,$03			; 
			defb $22,$6C,$88,$03			;
			defb $52,$70,$88,$05			; 
			...
			; Animate sequence for Pumps
			defb $1D,$1E,$1F,$FF			; $8860	 ; Tile Frames (x3), end
			defb $20,$21,$22,$FF			; $8864	 ; Tile Frames (x3), end			
			defb $1F,$1E,$1D,$FF			; $8868	 ; Tile Frames (x3), end		
			defb $22,$21,$20,$FF 			; $886C  ; Tile Frames (x3), end
			; Gun Emplcement left Part - Animate sequence for sliding doors
			defb $52,$52,$52,$52,$52,$52,$54,$56	
			defb $58,$58,$58,$58,$58,$58,$56,$54,$FF
			; Gun Emplcement right Part - Animate sequence for sliding doors
			defb $53,$53,$53,$53,$53,$53,$55,$57      
			defb $59,$59,$59,$59,$59,$59,$57,$55,$FF  
...
L_8A6C:		LD A,(HL)			; $8A6C ; get first item
			...
			OR $40				; $8A8D
			LD C,A				; $8A8F	 ; restore
			CALL COLOUR_SPRITE			; $8A90  ; flash colours
			JP L_8A6C			; $8A93
...
EMPTY_SLOT:		
			...
			LD B,(IX+$0B)				; $8CFB
BULLET_LOOP:	
			CALL BULLET_MOVEMENT		; $8CFE
			JR Z,SHOT_HIT_SCENE			; $8D01
			DJNZ BULLET_LOOP			; $8D03
			LD A,(IX+$0D)				; $8D05
			SUB (IX+$02)				; $8D08
			CP $14						; $8D0B
			JR C,CHECK_SCENE			; $8D0D
			CALL GET_CACHED_TILE_AS_HL	; $8D0F 
			LD A,(HL)					; $8D12 ; Test for Scene tile
			OR A						; $8D13
			JR NZ,SHOT_HIT_SCENE		; $8D14 ; Hit Scene
CHECK_SCENE:		
			LD A,(IX+$0A)				; $8D16	 ; ball frame
			...
			INC E						; $8D21
			INC E						; $8D22
			LD A,$03					; $8D23  ; type = Enemy bullet
			CALL PLAYER_HAZARD_LIST		; $8D25
			POP DE						; $8D28
			LD C,(IX+$0C)				; $8D29
			CALL COLOUR_SPRITE			; $8D2C
			JP ENEMY_SHOTS_LOOP			; $8D2F
SHOT_HIT_SCENE:
...
LOOP_EMPLACEMENT:
			...
			PUSH HL						; $8D47
			CALL GET_CACHED_TILE_AS_HL	; $8D48  ; 
			LD A,(HL)					; $8D4B
			OR A						; $8D4C
...
;-------------------------------------------------------------------------
; Store list of sprites (x, y, hitbox type)
PLAYER_HAZARD_LIST:  						; :)
			PUSH HL                			; 
			LD HL,(SPRITE_LIST_PTR)       	; current start ptr
			LD (HL),A              			; type (0-4)
			INC HL                 			; 
			LD (HL),E              			; Xpos
			INC HL                 			; 
			...
			INC HL               		  	; 
			LD (HL),$FF        		    	; end marker
			LD (SPRITE_LIST_PTR),HL     	; Update next start ptr
			POP HL                 			; 
			RET               		    	; 
;------------------------------------------------------------------------
...
WALK_LIST:	LD A,(HL)						; $8EF9
			...
			CALL SET_COLLISION_BOX						; $8EFD

			INC HL							; $8F00
			LD C,(HL)						; $8F01
			INC HL							; $8F02
			LD B,(HL)						; $8F03
			CALL CHECK_POINT_IN_HITBOX		; $8F04
			INC HL							; $8F07
			OR A							; $8F08
			JP Z,WALK_LIST					; $8F09
...
DO_GAME_OVER:
			...
			LD (ICON_LD_ADDR+1),HL			; $8F9D  ; Patch code with font gfx
			LD HL,GAME_OVER_TXT				; $8FA0
			LD BC,$0945						; $8FA3
			LD DE,$0E0B						; $8FA6  ; D=Y, E=X
TELE_TYPE:		
			PUSH BC							; $8FA9
			LD A,(HL)						; $8FAA  ; Get Char
			CALL ICON8x8					; $8FAB
			INC HL							; $8FAE
			INC E							; $8FAF
			LD BC,$2710						; $8FB0  ; pause each char
			...
			LD A,$07						; $8FBF  
PAUSE_LOOP:		
			CALL WAIT_21xBC					; $8FC1  ; x7 mini delays
			DEC A							; $8FC4
			JR NZ,PAUSE_LOOP				; $8FC5
			; ----------------------------------------
			CALL FILLOUT_HI_SCORE			; $8FC7
			JP DRAW_HI_SCORES				; $8FCA
; --------------------------------------------------------------------
GAME_OVER_TXT:
			defb "GAME OVER"				; $8FCD 
; --------------------------------------------------------------------

SPAWN_LANE_GUARDIANS:  
			; Guardians: these come in pairs and move synchronously
 			;  A = spawn trigger code ($F0-$F7)
			CP $F0						; $8FD6
			RET C						; $8FD8  ; exit <$F0
			CP $F8						; $8FD9
			RET NC						; $8FDB  ; exit >= $F8 
			...
			; Get next free guardian pair slot
			LD IX,(GUARDIAN_DATA)				; $8FE2  ; next free guardian
			LD HL,SPRITE24x16_DATA		; $8FE6
			LD (IX+$03),L				; $8FE9
			LD (IX+$04),H				; $8FEC
			LD (IX+$08),L				; $8FEF
			LD (IX+$09),H				; $8FF2
			LD (IX+$00),E				; $8FF5  ; Xpos
			LD (IX+$01),D				; $8FF8  ; Ypos
			LD (IX+$02),$3E				; $8FFB  ; starting frame (top guard)
			SUB $F0						; $8FFF  ; get here must be 0 to 7
			ADD A,A						; $9001  ; x2
			LD L,A						; $9002  ; Base
			LD H,$00					; $9003	
			LD BC,GUARDIAN_INIT_DATA	; $9005
			ADD HL,BC					; $9008  ; guardian data
			LD A,(HL)					; $9009  ; Ypos
			ADD A,D						; $900A  
			LD D,A						; $900B
			INC HL						; $900C
			LD A,(HL)					; $900D  ; 
			LD (IX+$0A),A				; $900E	 ; PLR_DIRECTION ($FF=up, $01=down)
			LD (IX+$05),E				; $9011  ; Xpos
			LD (IX+$06),D				; $9014  ; Ypos

			LD A,$42					; $9017  ; starting frame (bottom guard)
			LD (IX+$07),A				; $9019
			; Move to next guardian slot
			LD DE,$000B					; $901C
			ADD IX,DE					; $901F
			LD (IX+$00),$FF				; $9021
			LD (GUARDIAN_DATA),IX				; $9025  ;  Update next free guardian slot
			POP IX						; $9029
			POP HL						; $902B
			POP DE						; $902C
			POP BC						; $902D
...
GUARDIAN_INIT_DATA:						; $9030
			
			defb $3C,$FF	;   Y offset, PLR_DIRECTION ($FF=up, $01=down)			
			defb $37,$FF		
			defb $32,$FF
			defb $2D,$FF
			defb $3C,$01
...
LANE_GUARDIANS:		
			LD IX,GUARDIAN_DATA+2		; $9040
L_9044:		LD A,(IX+$00)				; $9044
			CP $FF						; $9047	  ; End marker
			RET Z						; $9049   ; Exit of list
			LD A,(IX+$0A)				; $904A   ; PLR_DIRECTION
			CP $FF						; $904D
			JP Z,GUARDIAN_UPWARD	    ; $904F
			JP GUARDIAN_DOWNWARD	    ; $9052
NEXT_GUARD_LOOP:		
			LD DE,$000B					; $9055
			ADD IX,DE					; $9058
			JP L_9044					; $905A
GUARDIAN_UPWARD:		
			...
			LD BC,$0747					; $908F ; ColR=$07, ColL=$47	 
			CALL COLOUR_16X16_SPRITE	; $9092
			...
			LD BC,$0747					; $90C1
			CALL COLOUR_16X16_SPRITE					; $90C4
			JP NEXT_GUARD_LOOP			; $90C7
GUARDIAN_DOWNWARD:		
			LD E,(IX+$05)				; $90CA
			LD D,(IX+$06)				; $90CD
			LD B,D						; $90D0
			...
			LD (IX+$08),L				; $90F1  ; consecutive sprite frames
			LD (IX+$09),H				; $90F4
			...
			LD (IX+$03),L				; $9123  ; consecutive sprite frames
			LD (IX+$04),H				; $9126
			...
			LD BC,$0747					; $912E
			CALL COLOUR_16X16_SPRITE					; $9131
			JP NEXT_GUARD_LOOP			; $9134
BOUNCE_GUARD:		
			; Reverse PLR_DIRECTION
			LD A,(IX+$0A)				; $9137  
			NEG							; $913A
			LD (IX+$0A),A				; $913C
			RET							; $913F
...
GUARDIAN_DATA:
			defb $42,$91                ; $9140  ; Next empty slot
			; Guardian pair structure (11 bytes each)
			; +00: Xpos
			; +01: Ypos
			; +02: frame
			; +03: Sprite data 
			...
			; +08: Sprite data
			; +09: Sprite data
			; +0A: PLR_DIRECTION ($FF = up, $01 = down)
			defb $00
			...
			defb $00,$00,$00,$00,$00,$FF                ; $91AB  ; $FF terminator

SPAWN_SNAKES:
			; --------------------------------------
 			; Tile value to triggers new snake segments
			CP $F8					; $91B1 
			RET C					; $91B3
			; --------------------------------------
			PUSH AF					; $91B4
			...
			LD A,(HL)				; $91D5
			ADD A,E					; $91D6
			LD (IX+$00),A			; $91D7  ; Xpos
			INC HL					; $91DA
			LD A,(HL)				; $91DB
			ADD A,D					; $91DC
			LD (IX+$01),A			; $91DD  ; Ypos
			INC HL					; $91E0
			LD A,(HL)				; $91E1
			LD (IX+$02),A			; $91E2  ; Direction
			INC HL					; $91E5
			LD A,(HL)				; $91E6
			LD (IX+$03),A			; $91E7  ; ??
...
SNAKES_TABLE:
			...
			defb $45,$78,$B0,$20,$45,$78,$B0,$20			; $9233
			defb $45,$78,$B0,$20,$45,$78,$B0,$20			; $923B
			defb $45,$78,$B0,$20,$45,$78,$B0,$20			; $9243
			defb $45,$78,$B0,$20,$45,$78,$B0,$20			; $924B
...
L_93EA:		PUSH DE				; $93EA
			...
			INC D				; $93F4
			INC D				; $93F5
			CALL SCENE_COLLISION_RIGHT				; $93F6
L_93F9:		OR A				; $93F9
			POP DE				; $93FA
			RET				; $93FB
L_93FC:		PUSH DE				; $93FC
			...
			INC D				; $9407
			INC D				; $9408
			CALL SCENE_COLLISION_LEFT				; $9409
L_940C:		OR A				; $940C
			POP DE				; $940D
			RET				; $940E
L_940F:		PUSH DE				; $940F
			...
			INC D				; $9419
			INC D				; $941A
			CALL SCENE_COLLISION_BOTTOM				; $941B
L_941E:		OR A				; $941E
			POP DE				; $941F
			RET					; $9420
L_9421:		PUSH DE				; $9421
...
DRAW_HI_SCORES:		
			...
			LD HL,HI_SCORE_DRAW_LIST	; $9439
			CALL DRAW_LIST				; $943C

			LD HL,FONT_DATA				; $943F
			LD (ICON_LD_ADDR+1),HL		; $9442
			LD HL,HI_SCORE_TEXT			; $9445
			LD B,$0A					; $9448  ; 10 display line
			LD C,$47					; $944A  ; Colour
			LD DE,$0B08					; $944C  ; D=Y, E=X
SCORE_LINE_LOOP:
			PUSH BC						; $944F
			LD B,$10					; $9450  ; 16 chars
PUT_CHARS:		
			LD A,(HL)				; $9452
			CALL ICON8x8				; $9453
			INC HL						; $9456  ; Next score char
			INC E						; $9457  ; Xpos
			DJNZ PUT_CHARS				; $9458
			POP BC						; $945A
			INC D						; $945B  ; Ypos
			LD E,$08					; $945C  ; Reset Xpos 
			DJNZ SCORE_LINE_LOOP		; $945E

SCROLL_WAIT_KEY_UP:
			; wait for all keys to be let go
			CALL ANY_KEY_DOWN			; $9460
			CALL NZ,SCROLL_BORDER		; $9463
			JP NZ,SCROLL_WAIT_KEY_UP	; $9466

			LD BC,$00AF					; $9469  ; Time to display
SCORE_TIMER_LOOP:
			PUSH BC						; $946C
			CALL SCROLL_BORDER			; $946D  ; (uses HALT for 50fps loop)
			CALL GET_KEY				; $9470
			OR A						; $9473
			POP BC						; $9474
			RET NZ						; $9475
...
L_97F3:
			...
			INC HL				; $97FA
			PUSH HL				; $97FB
			CALL GET_CACHED_TILE_AS_HL				; $97FC
			LD A,(HL)				; $97FF
			OR A				; $9800
			JR NZ,L_9807				; $9801
L_9803:		POP HL				; $9803
...
L_9939:		POP BC						; $9939
			...
			JP L_996E					; $9944
L_9947:		
			CALL GET_CACHED_TILE_AS_HL				; $9947
			LD A,(HL)				; $994A
			INC L				; $994B
			OR (HL)				; $994C
			PUSH BC				; $994D
...
FOUND_ITEM:		
			...
			INC HL							; $9A90
			LD D,(HL)						; $9A91
			CALL CHECK_POINT_IN_HITBOX		; $9A92
			POP HL							; $9A95
			OR A							; $9A96
...
; ??? jmp table to here, not found it yet!
			...
			LD DE,$2000				; $9ACB
			LD B,$14				; $9ACE
L_9AD0:		CALL GET_CACHED_TILE_AS_HL				; $9AD0
			LD A,(HL)				; $9AD3
			OR A				; $9AD4
			JR NZ,L_9AE0				; $9AD5
			LD (IX+$00),D				; $9AD7
...
; ???
			...
			LD DE,$2078				; $9AEB
			LD B,$14				; $9AEE
L_9AF0:		CALL GET_CACHED_TILE_AS_HL				; $9AF0
			LD A,(HL)				; $9AF3
			OR A				; $9AF4
			JR NZ,L_9B00				; $9AF5
			LD (IX+$00),D				; $9AF7
...
; ???
			...
			LD DE,$2000				; $9B0B
			LD B,$20				; $9B0E
L_9B10:		CALL GET_CACHED_TILE_AS_HL				; $9B10
			LD A,(HL)				; $9B13
			OR A				; $9B14
			JR NZ,L_9B20				; $9B15
			LD (IX+$00),E				; $9B17
...
L_9B20:		LD A,E				; $9B20
			...
			LD B,$20				; $9B4F
L_9B51:
			CALL GET_CACHED_TILE_AS_HL				; $9B51
			LD A,(HL)				; $9B54
			OR A				; $9B55
			JR NZ,L_9B61				; $9B56
			LD (IX+$00),E				; $9B58
...
PLACE_PICKUP:		
			...
			LD D,A				; $9FB2
			CALL GET_CACHED_TILE_AS_HL				; $9FB3
			LD A,(HL)				; $9FB6
			INC L				; $9FB7
			OR (HL)				; $9FB8
			PUSH DE				; $9FB9
...
L_A0A0:		LD E,(IX+$02)				; $A0A0
			...
			INC HL				; $A0B7
			LD B,(HL)				; $A0B8
			CALL COLOUR_16X16_SPRITE				; $A0B9
			LD A,(INPUT_ENABLED)				; $A0BC
			OR A				; $A0BF
			JR NZ,L_A0CC				; $A0C0
			LD BC,(POS_XY)				; $A0C2
			CALL CHECK_POINT_IN_HITBOX				; $A0C6
			OR A				; $A0C9
			JR NZ,L_A0D4				; $A0CA
...
SET_SCRN_ATTR:		
			...
			JR C,L_A453				; $A44F
			LD E,$00				; $A451
L_A453:		CALL GET_COLOUR_ATT_AS_HL				; $A453
L_A456:		PUSH HL				; $A456
			LD DE,$0300				; $A457
			ADD HL,DE				; $A45A
			EX DE,HL				; $A45B
...
COLOUR_16X16_SPRITE:		
			...
			LD A,E						; $A47F
			CP $80						; $A480
			JR C,CLAMP_XPOS				; $A482	 ; (logical xpos is half)
			LD E,$00					; $A484  
CLAMP_XPOS:	
			; These sprites (16x16) move only on the X-Axis
			; We still need 2+1 (8x3 pixels) for the 8 pixel boundry overlap
			LD A,$03					; $A486  ; Loop counter (rows)
			CALL GET_COLOUR_ATT_AS_HL	; $A488
COLOUR_LOOP:		
			EX AF,AF'					; $A48B
			PUSH HL						; $A48C
			LD DE,$0300					; $A48D
			...
			EX DE,HL					; $A491
			POP HL						; $A492
			; Write left attribute 
			LD A,(DE)					; $A493
			OR A						; $A494
			JR NZ,TOP_COL_OCCUPIED		; $A495  ; Don't colour anything used
			LD (HL),C					; $A497
TOP_COL_OCCUPIED:		
			INC HL						; $A498  ; next column
			INC DE						; $A499  ; next column
			; Write right attribute
			LD A,(DE)					; $A49A
			OR A						; $A49B
			JR NZ,BOTTOM_COL_OCCUPIED	; $A49C ; Don't colour anything used
			LD (HL),B					; $A49E
BOTTOM_COL_OCCUPIED:		
			LD DE,$001F					; $A49F  ; next row (32 bytes)
			ADD HL,DE					; $A4A2
			EX AF,AF'					; $A4A3
			DEC A						; $A4A4
			JP NZ,COLOUR_LOOP			; $A4A5
			POP HL						; $A4A8
			POP DE						; $A4A9
			POP BC						; $A4AA
			POP AF						; $A4AB
...
COLOUR_SPRITE:		
			...
			JR C,L_A4BA					; $A4B6
			LD E,$00					; $A4B8
L_A4BA:		CALL GET_COLOUR_ATT_AS_HL	; $A4BA
L_A4BD:		PUSH HL						; $A4BD
			LD DE,$0300					; $A4BE
			ADD HL,DE					; $A4C1
			EX DE,HL					; $A4C2
...
DRAW4X4SPRITE:		
			...
			ADD HL,HL				; $A4E6  ; x16
			ADD HL,HL				; $A4E7  ; x32  
			LD BC,GFX_DATA16X16		; $A4E8  ; sprites base
			ADD HL,BC				; $A4EB  ; base + offset
...
DRAW_TEXT:	
			...
			LD BC,GFX_DATA16X16			; $A557 ; Tile Data
...
SET_TILE16X16_COL:
			...
			LD B,H				; $A58F
			LD C,L				; $A590
			CALL GET_COLOUR_ATT_AS_HL			; $A591
...
MAP01:		defb $01                                    ; $A5AA 
			...
			defb $48,$FF,$05,$14,$13,$FF,$05,$00		; $A5BB 
			defb $15,$01,$02,$03,$15,$00,$1D,$00		; $A5C3 
			defb $1F,$FF,$05,$00,$10,$14,$4B,$01		; $A5CB 
			...
			defb $15,$FF,$09,$00,$12,$14,$49,$01		; $A5F3 
			defb $02,$03,$15,$FF,$0A,$00,$1D,$15		; $A5FB 
			defb $03,$04,$04,$15,$00,$00,$96,$0D		; $A603 
			defb $80,$81,$82,$00,$0F,$0E,$0D,$4A		; $A60B  ; $80,$81,$82 Volcano tiles
			defb $14,$14,$01,$15,$00,$00,$15,$01		; $A613 
			defb $02,$03,$04,$05,$01,$04,$03,$01		; $A61B 
			defb $01,$02								; $A623
MAP02:		defb $02,$03,$04,$05,$02,$03	
			defb $04,$05,$15,$01,$01,$02,$03,$04		; $A62B 
			defb $05,$02,$FF,$08,$14,$13,$34,$35		; $A633 
			defb $34,$35,$34,$35,$34,$7C,$00,$4E		; $A63B 
			defb $4F,$FF,$0C,$00,$7E,$00,$50,$51		; $A643 
			defb $FF,$0C,$00,$7E,$00,$52,$53,$FF		; $A64B 
			...
			defb $10,$14,$14,$7C,$FF,$05,$00,$79		; $A673 
			defb $FF,$05,$78,$79,$15,$01,$01			; $A67B 
MAP03:		defb $01
			defb $02,$03,$01,$02,$03,$04,$05,$01		; $A683 
			defb $03,$04,$05,$15,$01,$02,$03,$34		; $A68B 
			defb $35,$36,$35,$36,$37,$36,$35,$34		; $A693 
			defb $35,$35,$34,$15,$04,$05,$01,$FF		; $A69B 
			defb $0C,$00,$12,$14,$14,$14,$FF,$0D		; $A6A3 
			defb $00,$30,$31,$FF,$0E,$00,$32,$33		; $A6AB 
...
MAP04:		defb $01,$01,$04,$01,$15,$19,$1A,$FF		; $A6D3 
...
MAP05:		defb $FF									; $A73B
			...
			defb $4A,$FF,$06,$14,$49,$2F,$2E,$48		; $A7A3 
			defb $14,$4B,$01,$02,$03,$04,$01,$02		; $A7AB 
			defb $03,$01,$01,$02,$15,$2E,$2F,$15		; $A7B3 
			defb $01,$02		
MAP06:		defb $FF,$A0,$00							; $A7BD
MAP07: 		defb $01,$15,$00							; $A7C0	
			defb $00,$15,$02,$03,$04,$01,$02,$02		; $A7C3 
			defb $01,$02,$01,$01,$02,$01,$15,$00		; $A7CB 
			defb $00,$12,$FF,$08,$14,$49,$03,$02		; $A7D3 
			defb $02,$15,$FF,$0B,$00,$15,$02,$03		; $A7DB 
			defb $03,$15,$FF,$09,$00,$10,$14,$4B		; $A7E3 
			defb $02,$03,$04,$15,$FF,$09,$00,$15		; $A7EB 
			defb $01,$01,$01,$02,$04,$15,$FF,$09		; $A7F3 
			defb $00,$15,$01,$02,$03,$04,$01,$15		; $A7FB 
			defb $FF,$09,$00,$12,$14,$14,$49,$01		; $A803 
			defb $02,$15,$EB,$FF,$0B,$00,$15,$01		; $A80B 
			defb $03,$4A,$14,$11,$FF,$08,$00,$10		; $A813 
			defb $14,$4B,$01,$04,$01,$02,$15,$00		; $A81B 
			defb $00,$ED,$ED,$FF,$04,$00,$15,$02		; $A823 
			defb $01,$02								; $A82B 
MAP08:		defb $7C,$FF,$05,$00,$79,$FF				; $A82D
			defb $05,$78,$79,$15,$03,$01,$7D,$FF		; $A833 
			...
			defb $FF,$07,$14,$4B,$02,$01,$7F,$01		; $A893 
			defb $02,$03,$04,$15,$01,$02,$03,$04		; $A89B 
			defb $01,$02,$03,$04,$01,$02
MAP09:		defb $01,$02								; $A8A9
			defb $03,$04,$01,$02,$03,$04,$01,$02		; $A8AB 
			defb $03,$04,$02,$01,$03,$01,$FF,$10		; $A8B3 
			defb $14,$FF,$0A,$00,$47,$FF,$04,$00		; $A8BB 
			...
			defb $FF,$06,$00,$7D,$02,$03,$4A,$FF		; $A8E3 
			defb $07,$14,$11,$00,$00,$00,$10,$14		; $A8EB 
			defb $01,$02,$03,$04,$05,$01,$02,$03		; $A8F3 
			defb $04,$05,$15,$00,$00,$00,$15,$01		; $A8FB 
MAP10:		defb $01,$02,$03,$04,$01,$15,$79,$FF		; $A903 
			defb $07,$78,$79,$11,$01,$48,$14,$14		; $A90B 
			defb $14,$13,$00,$00,$30,$31,$FF,$05		; $A913 
			defb $00,$15,$02,$15,$FF,$06,$00,$32		; $A91B 
			defb $33,$FF,$05,$00,$15,$03,$15,$FF		; $A923 
			...
			defb $00,$10,$FF,$06,$14,$4B
MAP11:		defb $01,$02								; $A971
			defb $15,$04,$01,$02,$03,$15,$02,$03		; $A973 
			defb $15,$00,$00,$15,$01,$02,$01,$02		; $A97B 
			defb $15,$34,$35,$36,$34,$12,$14,$14		; $A983 
			defb $13,$00,$00,$12,$14,$49,$48,$14		; $A98B 
			...
			defb $14,$4B,$15,$FF,$05,$00,$ED,$ED		; $A9BB 
			defb $ED,$00,$00,$00,$15,$01,$02,$03		; $A9C3 
MAP12:		defb $FF,$A0,$00
MAP13:      defb $01,$02,$03,$15,$FF		
			defb $08,$00,$15,$01,$03,$02,$02,$01		; $A9D3 
			defb $02,$15,$FF,$08,$00,$15,$01,$02		; $A9DB 
			defb $03,$01,$48,$14,$13,$FF,$08,$00		; $A9E3 
			defb $12,$14,$49,$01,$02,$15,$FF,$0C		; $A9EB 
			defb $00,$15,$02,$03,$15,$20,$FF,$0B		; $A9F3 
			defb $00,$15,$02,$04,$4A,$14,$11,$FF		; $A9FB 
			...
			defb $14,$4B,$19,$1A,$FF,$0A,$1B			; $AA2B 
MAP14:		defb $01
			defb $02,$04,$03,$01,$15,$04,$01,$02		; $AA33 
			defb $03,$04,$05,$01,$02,$03,$04,$05		; $AA3B 
			defb $04,$03,$03,$02,$15,$34,$35,$36		; $AA43 
			defb $37,$35,$36,$35,$37,$35,$34,$02		; $AA4B 
			...
			defb $46,$46,$00,$00,$92,$93,$92,$93		; $AA83 
			defb $92,$FF,$04,$00,$0E,$3D,$0C,$4C		; $AA8B 
			defb $4D,$0D,$FF,$07,$1B,$1C,$19,$0C		; $AA93 
			defb $02,$02,$03,$04,$05,$01				; $AA9B 
MAP15:		defb $01,$02		
			defb $15,$04,$15,$FF,$05,$78,$79,$00		; $AAA3 
			defb $00,$00,$15,$01,$34,$34,$12,$14		; $AAAB 
			...
			defb $19,$16,$17,$18,$15,$01				; $AB13 
MAP16:		defb $01,$04		
			defb $03,$02,$01,$15,$00,$00,$15,$01		; $AB1B 
			defb $02,$03,$04,$04,$04,$01,$02,$48		; $AB23 
			defb $14,$14,$14,$4B,$00,$00,$4A,$14		; $AB2B 
			defb $14,$14,$49,$03,$03,$02,$03,$15		; $AB33 
			...
			defb $FA,$FF,$05,$00,$03,$15,$00,$46		; $AB5B 
			defb $46,$00,$00,$46,$46,$00,$00,$00		; $AB63 
			defb $10,$14,$14,$14,$04,$15,$00,$4C		; $AB6B 
			defb $4D,$00,$46,$4C,$4D,$00,$00,$00		; $AB73 
			defb $15,$01,$02,$03,$04,$4A,$FF,$0A		; $AB7B 
			defb $14,$4B,$04,$01,$02,$01,$02,$03		; $AB83 
			defb $04,$01,$02,$04,$04,$01,$02,$03		; $AB8B
			defb $04,$01,$02,$03,$04
MAP17:		defb $15,$FF,$0B							; 
			defb $00,$15,$01,$02,$03,$15,$8D,$FF		; $AB9B 
			defb $0A,$00,$15,$01,$02,$03,$15,$90		; $ABA3 
			defb $FF,$0A,$00,$12,$14,$49,$03,$15		; $ABAB 
			...
			defb $00,$46,$46,$00,$23,$24,$00,$FD		; $ABD3 
			defb $15,$04,$15,$8D,$8F,$8D,$8F,$8D		; $ABDB 
			defb $8D,$4C,$4D,$00,$25,$26,$00,$FD		; $ABE3 
			defb $15,$04,$4A,$FF,$0D,$14,$4B,$04		; $ABEB 
			defb $01,$02,$02,$03,$04,$01,$02,$03		; $ABF3 
			defb $04,$01,$03,$04,$01,$02,$03,$04		; $ABFB 
MAP18:		defb $FF,$A0,$00
MAP19:		defb $01,$02,$48,$FF,$0D		
			...
			defb $02,$15,$00,$10,$FF,$08,$14,$4B		; $AC8B 
			defb $01,$02
MAP20:		defb $FF,$0D,$14,$49,$01,$02				; $AC95
			defb $00,$00,$47,$47,$47,$FF,$05,$00		; $AC9B
			defb $30,$31,$00,$15,$03,$04,$14,$11		; $ACA3
			defb $FF,$08,$00,$32,$33,$00,$15,$01		; $ACAB
			...
			defb $FF,$06,$14,$4B,$03,$03,$03,$15		; $ACF3
			defb $FF,$04,$00,$15,$01,$02,$03,$04		; $ACFB
			defb $05,$01,$02,$01,$02					; $AD03
MAP21:		defb $01,$02,$03		
			defb $04,$03,$02,$01,$04,$03,$02,$01		; $AD0B
			defb $05,$04,$03,$01,$02,$01,$01,$02		; $AD13
			defb $03,$48,$FF,$0B,$14,$02,$01,$03		; $AD1B
			defb $02,$15,$00,$4E,$4F,$FF,$08,$00		; $AD23
			defb $03,$48,$14,$14,$13,$00,$50,$51		; $AD2B
			defb $FF,$08,$00,$04,$15,$FF,$04,$00		; $AD33
			defb $52,$53,$FF,$08,$00,$01,$15,$FF		; $AD3B
			...
			defb $04,$4A,$14,$14,$49,$8F,$8D,$8F		; $AD63
			defb $8D,$8F,$8D,$8F,$48,$14,$14,$14		; $AD6B
			defb $01,$02,$03,$01,$15,$8D,$8F,$8D		; $AD73
			defb $8F,$8D,$8F,$8D,$15,$01,$02,$03		; $AD7B
MAP22:		defb $01,$02,$03,$04,$05,$02,$03,$04		; $AD83
			defb $05,$01,$02,$03,$04,$05,$02,$01		; $AD8B
			defb $FF,$06,$14,$9A,$9B,$9C,$FF,$05		; $AD93
			defb $14,$49,$01,$FF,$07,$00,$47,$FF		; $AD9B
			defb $06,$00,$15,$02,$FF,$0E,$00,$15		; $ADA3
			defb $03,$FF,$0C,$00,$0B,$00,$15,$04		; $ADAB
			defb $FF,$06,$00,$A0,$A1,$FF,$04,$00		; $ADB3
			defb $0A,$00,$15,$05,$FF,$04,$00,$46		; $ADBB
			defb $46,$A2,$A3,$46,$00,$00,$00,$09		; $ADC3
			defb $00,$15,$02,$00,$00,$E9,$00,$4E		; $ADCB
			defb $4F,$A4,$A5,$4E,$4F,$00,$06,$07		; $ADD3
			defb $08,$15,$03,$14,$14,$14,$9A,$FF		; $ADDB 
			defb $06,$9B,$9C,$14,$14,$14,$4B,$04		; $ADE3 
			defb $05,$02,$03,$04,$05,$01,$02,$03		; $ADEB 
			defb $01,$02,$02,$03,$04					; $ADF3 
			defb $05,$02,$05		
MAP23:		defb $04,$02,$03,$04,$02,$01,$03,$04		; $ADFB 
			defb $01,$02,$05,$04,$01,$02,$03,$01		; $AE03 
			defb $03,$01,$48,$FF,$0D,$14,$01,$02		; $AE0B 
...
MAP24:		defb $01,$15		
			...
			defb $ED,$ED,$FF,$09,$00,$15,$02			; $AECB 
MAP25:		defb $01	
			defb $02,$9D,$00,$15,$01,$02,$01,$02		; $AED3 
			defb $04,$01,$02,$03,$04,$01,$03,$03		; $AEDB 
			defb $04,$9F,$00,$15,$01,$02,$03,$04		; $AEE3 
			defb $01,$02,$03,$04,$01,$02,$02,$78		; $AEEB 
			defb $78,$79,$00,$12,$14,$14,$14,$9A		; $AEF3 
			defb $9B,$9C,$14,$14,$14,$49,$01,$01		; $AEFB 
			defb $79,$FF,$0C,$00,$15,$04,$02,$9D		; $AF03 
			defb $FF,$0C,$00,$9D,$03,$03,$9E,$FF		; $AF0B 
			defb $0C,$00,$9E,$01,$04,$9E,$FF,$0C		; $AF13 
			defb $00,$9F,$01,$04,$9F,$FF,$0C,$00		; $AF1B 
			defb $15,$04,$01,$79,$78,$79,$FF,$08		; $AF23 
			defb $00,$10,$14,$4B,$04,$02,$03,$04		; $AF2B 
			defb $9D,$00,$00,$ED,$ED,$ED,$00,$00		; $AF33 
			defb $00,$15,$01,$02,$03					; $AF3B 
MAP26:		defb $04,$15,$00		
...
MAP28:		defb $03,$02,$01,$04		 
			...
			defb $2E,$2F,$2E,$2F,$4A,$14,$49,$02		; $B093 
			defb $01,$4A,$FF,$04,$14,$49,$2E,$2F		; $B09B 
...
MAP32:		defb $01,$02,$04,$03,$15,$16	
			...
			defb $0E,$00,$4C,$4D,$00,$00,$00,$E9		; $B26B 
			defb $00,$EA,$FF,$05,$00,$FF,$05,$1B		; $B273 
			defb $1C,$19,$16,$17,$18,$16,$17,$18		; $B27B 
			defb $16,$17,$18							; $B283 
MAP33:		defb $01,$02,$03,$15,$01
			defb $02,$03,$04,$01,$02,$03,$04,$02		; $B28B 
			defb $03,$04,$05,$04,$05,$04,$15,$34		; $B293 
			defb $35,$36,$37,$35,$34,$35,$36,$37		; $B29B 
			defb $4E,$4F,$36,$14,$14,$14,$13,$FF		; $B2A3 
			defb $09,$00,$50,$51,$FF,$0E,$00,$52		; $B2AB 
			defb $53,$FF,$2F,$00,$A0,$A1,$FF,$08		; $B2B3 
			defb $00,$27,$28,$00,$46,$46,$00,$A2		; $B2BB 
			defb $A3,$FF,$08,$00,$29,$2A,$46,$4C		; $B2C3 
			defb $4D,$46,$A4,$A5,$16,$17,$18,$16		; $B2CB 
			defb $17,$18,$19,$1A,$FF,$08,$1B		; $B2D3 
MAP34:		defb $01
			defb $01,$15,$03,$01,$02,$9E,$FF,$07		; $B2DB 
			defb $00,$15,$01,$34,$00,$15,$03,$04		; $B2E3 
...
MAP35:		defb $01,$15,$01,$02,$03,$04,$3B,$02		; $B35B 
			defb $3B,$04,$3B,$02,$3B,$04,$15,$01		; $B363 
			defb $02,$15,$35,$4E,$4F,$35,$3B,$35		; $B36B 
			defb $3B,$35,$3B,$35,$3D,$34,$15,$02		; $B373 
			defb $03,$15,$00,$50,$51,$00,$3D,$00		; $B37B 
			defb $3B,$00,$3D,$00,$47,$00,$15,$03		; $B383 
			defb $04,$15,$00,$52,$53,$00,$47,$00		; $B38B 
			defb $3D,$00,$47,$00,$00,$00,$15,$04		; $B393 
			defb $01,$15,$FF,$06,$00,$47,$FF,$05		; $B39B 
...
MAP36:		defb $01,$15,$FF	
			...
			defb $15									; $B44B 
MAP37:		defb $01,$15,$01,$02,$03,$04,$15		
			defb $01,$02,$03,$15,$05,$04,$03,$02		; $B453 
			defb $01,$02,$15,$34,$35,$36,$37,$15		; $B45B 
			defb $04,$01,$02,$15,$34,$35,$36,$37		; $B463 
			defb $34,$03,$15,$FF,$04,$00,$12,$14		; $B46B 
			defb $14,$14,$13,$FF,$05,$00,$04,$15		; $B473 
			defb $FF,$05,$00,$47,$47,$FF,$07,$00		; $B47B 
			...
			defb $00,$87,$00,$00,$87,$00,$46,$46		; $B4A3 
			defb $FF,$06,$00,$01,$15,$00,$88,$89		; $B4AB 
			defb $8A,$88,$00,$4C,$4D,$16,$17,$18		; $B4B3 
			defb $16,$17,$18,$02,$9A,$FF,$0D,$9B		; $B4BB 
			defb $9C									; $B4C3 
MAP38:		defb $01,$02,$03,$04,$01,$02,$03	
			defb $04,$01,$02,$01,$04,$01,$02,$03		; $B4CB 
			defb $04,$34,$35,$36,$37,$34,$35,$35		; $B4D3 
			defb $36,$34,$3C,$35,$3C,$36,$34,$35		; $B4DB 
			defb $34,$FF,$09,$00,$3B,$00,$3B,$FF		; $B4E3 
			...
			defb $16,$17,$16,$17,$FF,$10,$14			; $B51B 
MAP39:		defb $01	
			defb $04,$01,$02,$04,$03,$02,$01,$04		; $B523 
			defb $03,$02,$01,$04,$02,$03,$01,$34		; $B52B 
			defb $35,$36,$37,$34,$35,$35,$35,$36		; $B533 
			defb $37,$34,$35,$36,$37,$34,$34,$EE		; $B53B 
			defb $EE,$EE,$FF,$1C,$00,$96,$FF,$0F		; $B543 
			defb $00,$15,$00,$00,$00,$A0,$A1,$FF		; $B54B 
			defb $0A,$00,$15,$00,$00,$00,$A2,$A3		; $B553 
			defb $00,$00,$46,$46,$00,$46,$46,$00		; $B55B 
			defb $00,$00,$15,$00,$00,$00,$A4,$A5		; $B563 
			defb $00,$00,$4E,$4F,$46,$4E,$4F,$00		; $B56B 
			defb $00,$00,$15,$19,$1A,$FF,$0A,$1B		; $B573 
			defb $1C,$19,$1A,$4B,$01,$02,$03,$04		; $B57B 
			defb $05,$01,$02,$03,$04,$05,$01,$02		; $B583 
			defb $03,$04,$05,$04						; $B58B 
MAP40:		defb $01,$02,$03,$04	
			defb $01,$02,$03,$04,$01,$04,$01,$01		; $B593 
			defb $02,$01,$02,$01,$FF,$07,$14,$49		; $B59B 
...
MAP41:		defb $01,$15,$00,$00,$00	
			...
			defb $14,$4B,$2E,$2F,$2E,$2F,$2E,$15		; $B64B 
			...
			defb $01,$FF,$0E,$14,$4B,$02,$01,$02		; $B683 
			defb $03,$04,$05,$01,$02,$03,$04,$05		; $B68B 
			defb $01,$02,$03,$04,$03,$04				; $B693 
MAP42:		defb $FF,$A0	
			defb $00									; $B69B 
MAP43:		defb $01,$02,$03,$3B,$05,$3B,$02	
			defb $03,$04,$05,$01,$A9,$03,$04,$05		; $B6A3 
			defb $01,$01,$48,$A8,$3B,$A8,$3B,$FF		; $B6AB 
			defb $05,$A8,$13,$34,$35,$36,$37,$02		; $B6B3 
			defb $A9,$47,$3D,$00,$3B,$00,$00,$4E		; $B6BB 
			defb $4F,$FF,$06,$00,$03,$A9,$00,$47		; $B6C3 
			defb $00,$3D,$00,$00,$50,$51,$FF,$06		; $B6CB 
			defb $00,$04,$A9,$00,$00,$00,$47,$00		; $B6D3 
			defb $00,$52,$53,$FF,$06,$00,$05,$A9		; $B6DB 
			...
			defb $01,$02,$03						; $B713 
MAP44:		defb $01,$02,$03,$04,$05	
			defb $01,$02,$03,$04,$05,$01,$02,$03		; $B71B 
			defb $04,$15,$01,$34,$35,$A6,$A7,$34		; $B723 
			defb $A6,$A7,$34,$35,$36,$34,$35,$37		; $B72B 
			defb $34,$15,$01,$00,$00,$47,$47,$00		; $B733 
			defb $47,$47,$FF,$07,$00,$15,$02,$FF		; $B73B 
			defb $0E,$00,$15,$03,$FF,$0E,$00,$15		; $B743 
			defb $04,$FF,$0B,$00,$10,$14,$14,$4B		; $B74B 
			defb $01,$FF,$0B,$00,$15,$01,$02,$03		; $B753 
			defb $01,$00,$00,$46,$46,$EB,$46,$46		; $B75B 
			defb $FF,$04,$00,$15,$04,$05,$01,$02		; $B763 
			defb $0C,$0D,$4E,$4F,$0D,$4E,$4F,$0E		; $B76B 
			defb $0C,$0D,$0E,$15,$02,$03,$04,$03		; $B773 
			defb $01,$02,$03,$04,$01,$02,$03,$04		; $B77B 
			defb $05,$01,$02,$15,$04,$05,$02,$04		; $B783 
MAP45:		defb $01,$02,$03,$04,$01,$02,$03,$04		; $B78B 
			defb $05,$01,$02,$05,$04,$01,$02,$03		; $B793 
			defb $04,$48,$FF,$0E,$A8,$03,$A9,$00		; $B79B 
			defb $1D,$1F,$00,$00,$30,$31,$FF,$07		; $B7A3 
			defb $00,$02,$A9,$FF,$05,$00,$32,$33		; $B7AB 
			...
			defb $7C,$01,$A9,$FF,$0D,$00,$7D			; $B7D3 
MAP46:		defb $01	
...
MAP55:		defb $01,$02,$03,$04,$05,$01			 
			...
			defb $01,$02,$03
MAP56: 		defb $01,$02,$A9,$01,$02		
			defb $03,$3B,$34,$A9,$01,$02,$03,$A9		; $BB8B 
			defb $00,$15,$01,$03,$04,$A9,$34,$35		; $BB93 
			defb $36,$3B,$00,$A9,$04,$05,$01,$A9		; $BB9B 
			...
			defb $82,$10,$14,$4B,$01,$01,$02,$03		; $BC03 
...
MAP57:	    defb $05,$A9,$EF		
			...
			defb $FF,$0D,$00,$7D,$03,$A9,$00,$46		; $BC3B 
			defb $46,$00,$00,$00,$46,$46,$FF,$05		; $BC43 
			defb $00,$7E,$02,$A9,$46,$4C,$4D,$00		; $BC4B 
			defb $46,$00,$4C,$4D,$46,$FF,$04,$00		; $BC53 
			defb $7C,$01,$4A,$FF,$09,$A8,$11,$00		; $BC5B 
			defb $00,$00,$7E,$01,$02,$03,$04,$05		; $BC63 
			defb $01,$02,$03,$04,$05,$01,$A9,$00		; $BC6B 
			defb $00,$00,$7F
MAP58:      defb $01,$02,$15,$FF,$0B		
			...
			defb $07,$00,$7D,$05
MAP59:		defb $01,$01,$01,$02		
			defb $03,$04,$05,$01,$02,$03,$15,$01		; $BCDB 
			defb $02,$A9,$01,$01,$02,$02,$48,$FF		; $BCE3 
			defb $07,$14,$13,$34,$36,$A9,$02,$02		; $BCEB 
			...
			defb $00,$03,$04,$15,$FF,$08,$00,$46		; $BD1B 
			defb $46,$00,$00,$00,$04,$05,$15,$FF		; $BD23 
			defb $07,$00,$10,$FF,$05,$A8,$01,$01		; $BD2B 
			defb $15,$FF,$07,$00,$A9,$01,$04,$03		; $BD33 
			defb $02,$01
MAP60:      defb $01,$02,$01,$A9,$FF,$0A		
			...
			defb $9C,$FF,$04,$A8,$4B,$03,$01,$02		; $BDB3 
			defb $03,$04,$05,$01,$02,$03,$04,$05		; $BDBB 
			defb $01,$02,$03,$04,$05,$01
MAP61: 		defb $01,$A9		
...
MAP62:		defb $01,$A9,$FF,$0B,$00,$15,$01,$02		; $BE33 
			...
			defb $00,$8D,$00,$8D,$00,$00,$30,$31		; $BE63 
			defb $05,$A9,$FF,$0C,$00,$32,$33,$01		; $BE6B 
			defb $A9,$FF,$06,$00,$46,$46,$FF,$06		; $BE73 
			defb $00,$03,$A9,$FF,$05,$00,$46,$4C		; $BE7B 
			defb $4D,$46,$FF,$05,$00,$03,$A9,$00		; $BE83 
			defb $00,$00,$46,$9A,$FF,$04,$9B,$9C		; $BE8B 
			defb $46,$00,$00,$00,$01,$4A,$A8,$A8		; $BE93 
			defb $A8,$9A,$FF,$06,$9B,$9C,$A8,$A8		; $BE9B 
			defb $A8,$01,$02,$03,$04,$05,$01,$02		; $BEA3 
			defb $03,$04,$05,$01,$03,$04,$05,$01		; $BEAB 
			defb $02
MAP63:		defb $01,$02,$03,$04,$15,$01,$02			
			defb $03,$04,$05,$01,$02,$03,$04,$03		; $BEBB 
			defb $04,$05,$01,$03,$02,$15,$34,$35		; $BEC3 
			defb $36,$37,$34,$36,$34,$35,$37,$36		; $BECB 
			defb $34,$FF,$04,$14,$13,$FF,$18,$00		; $BED3 
			defb $EC,$EC,$EC,$FF,$12,$00,$A0,$A1		; $BEDB 
			defb $FF,$0E,$00,$A2,$A3,$00,$46,$46		; $BEE3 
			defb $00,$46,$46,$00,$00,$27,$28,$FF		; $BEEB 
			defb $04,$00,$A4,$A5,$46,$4C,$4D,$46		; $BEF3 
			defb $4C,$4D,$46,$00,$29,$2A,$00,$00		; $BEFB 
			defb $FF,$10,$A8,$01,$02,$03,$04,$05		; $BF03 
			defb $01,$01,$02,$03,$04,$05,$01,$02		; $BF0B 
			defb $03,$04,$01
MAP64:		defb $01,$02,$03,$04,$01					
			defb $02,$02,$01,$02,$34,$15,$01,$02		; $BF1B 
			defb $03,$04,$01,$34,$35,$36,$37,$35		; $BF23 
			defb $34,$35,$36,$35,$00,$12,$14,$9A		; $BF2B 
			...
			defb $04,$05,$02
MAP65:		defb $01,$01,$02,$03,$04					
			defb $04,$04,$01,$02,$03,$04,$15,$00		; $BF63 
			defb $00,$00,$7C,$14,$9A,$FF,$07,$9B		; $BF6B 
			defb $9C,$14,$13,$00,$00,$00,$7D,$FF		; $BF73 
			defb $06,$00,$30,$31,$00,$30,$31,$FF		; $BF7B 
			defb $04,$00,$7E,$FF,$06,$00,$32,$33		; $BF83 
			defb $00,$32,$33,$FF,$04,$00,$7E,$FF		; $BF8B 
			...
			defb $7F
MAP66:		defb $04,$05,$15,$FF,$0B,$00,$9D			
...
MAP68:		defb $01,$02,$03,$04,$01,$02,$03,$04	; $C0AB
...
MAP69: 		defb $01,$3B,$03,$3B,$05,$3B			
			...
			defb $02,$01,$14,$49,$00,$3B,$00,$3B	; $C183
			defb $00,$3B,$00,$3B,$00,$00,$00,$15	; $C18B
			defb $03,$05,$01,$15,$0D,$3B,$0D,$3B	; $C193
			defb $0F,$3B,$0C,$3B,$0E,$0C,$0E,$15	; $C19B
			defb $04,$01
MAP70: 		defb $7B,$02,$03,$04,$3B,$34			
			defb $3B,$36,$3B,$34,$3B,$36,$3B,$04	; $C1AB
			defb $05,$01,$7C,$34,$35,$36,$3B,$00	; $C1B3
			...
			defb $46,$00,$7E,$8B,$00,$00,$8B,$00	; $C203
...
FONT_DATA:
			...
			defb $02,$01							; $C2FB
MAP73:		defb $01,$02,$03,$04,$05,$01			
			defb $02,$03,$04,$05,$15,$01,$01,$02	; $C303
			defb $03,$04,$34,$36,$34,$35,$36,$37	; $C30B
			defb $35,$36,$37,$37,$15,$03,$04,$03	; $C313
...
MAP74:		defb $01,$02,$15,$FF,$0B,$00			
			...
			defb $9C,$FF,$04,$A8,$4B,$04,$01,$02	; $C3DB
